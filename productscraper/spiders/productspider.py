import scrapy
from productscraper.items import BookItem
...
class ProductspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for book in books:
            ...
            if 'catalogue' in relative_url: # correction added as pages should start with catalogue
                book_url = 'https://books.toscrape.com/' + relative_url
            else:
                book_url = 'https://books.toscrape.com/catalogue/' + relative_url
            yield response.follow(book_url, callback=self.parse_book_content)


        next_page = response.css('li.next a::attr(href)').get()

        if next_page is not None:
            if 'catalogue' in next_page: # correction added as pages should start with catalogue
